...
const Dashboard = ({ onHideDashboard, valores }) => {
  const handleDownload = (mesanoreferencia) => {
    ...
    link.href = process.env.PUBLIC_URL + `/${mesanoreferencia}`; // Caminho para o arquivo PDF com mesanoreferencia
    link.download = `${mesanoreferencia}.pdf`; // Nome do arquivo para download
...
function App() {
  ...
  useEffect(() => {
    async function getValores() {
      const response = await axios.get("https://cors-anywhere.herokuapp.com/https://projeto-lumi-back-ajrx8b3q2-devjjs-projects-dd81c9bd.vercel.app/lumi");
